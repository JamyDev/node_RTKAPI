...
function RTKServer (info, cb) {
        ...
        typeof info.salt === 'string')
    {
        ...
        var connection = net.connect({port: $global.port, host: $global.host, allowHalfOpen: true}, function (con) {
            afterConnect();
        });
    ...
    function off (event, token, fn) {
        ...
        if (getSize($local.listeners.l_on[event]) === 0 && getSize($local.listeners.l_once[event]) === 0) {
            var p = new packet
                .Packet($global.encrypted, $local.key, $local.eventsid, 'unregisterEventListener', {classes: event}, '')
                .compile(function (b) {
                    if ($global.connected)
                        connection.write(b);
                    else
    ...
    function reconnect () {
        $local.ee.emit('status', {oldStatus: $global.status, newStatus: 1});
        $global.status = 1;
        connection = net.connect({port: $global.port, host: $global.host, allowHalfOpen: true}, function (con) {
            afterConnect();
        });
        handleConnection();
    }
    ...
    function afterConnect() {
        $local.transform = transform.construct($local.ee);
        connection.pipe($local.transform);
        $global.errorCount = 0;
    ...
    function afterPersistent () {
        ...
        // Runs code that n$local.eeds to happen if connection is persistent
        ...
        if (oldListeners.length) { // If there were listeners before, re-register them (without the internal listeners)
            var p = new packet
            .Packet($global.encrypted, $local.key, $local.eventsid, 'registerEventListener', {classes: oldListeners.join(',')}, '')
            .compile(function (b) {
                if ($global.connected) {
                    connection.write(b);
                }
